...
export const useChat = () => {
  ...
  const getOptions = () => {
    ...
    return hasSelectedOption ? [...base, { key: 'newChat', label: i18n.t('chat.newChat') }] : base;
  };
        ...
        if (response.ok) {
          ...
          if (isPresent) {
            ...
            setMessages(prev => [...prev, {
              id: getUniqueId(),
              role: 'assistant',
              content: i18n.t('chat.websitePositionNextPage', { position: position + 10, query: searchQuery }),
              type: 'text'
            },
          {
              id: getUniqueId(),  
  ...
  const handleSendInput = async (val: string) => {
    ...
    if (!currentUrl) {
      ...
      if (waitingFor === 'seo' || waitingFor === 'brand') {
        ...
        const contactKeywords = ['hubungi kami', 'kontak essentials', 'whatsapp', 'kontak', 'hubungi', 'perbaiki', 'bantuan', 'help', 'support', 'customer service', 'layanan pelanggan', 'customer care', 'fixing', 'fix', 'pelayanan', 'dukungan', 'servis', 'assistance', 'assisten', 'service', 'supporting'];
        if (contactKeywords.some(keyword => val.toLowerCase().includes(keyword))) {
          setMessages(prev => [
            ...prev,
            { id: getUniqueId(), role: 'user', content: val, type: 'text' }
          ]);
          ...
          setTimeout(() => {
            setMessages(prev => [
              ...prev,
              {
                id: getUniqueId(),
                role: 'assistant',
                content: 'Bisa hubungi kami ke WhatsApp berikut',
                type: 'text'
              },
              {
                id: getUniqueId(),
