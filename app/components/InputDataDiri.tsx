...
export default function InputDataDiri({ onSubmitSuccess }: InputDataDiriProps) {
  ...
  return (
    <div className="bg-white/10 p-8 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] w-full max-w-md mx-4 border border-white/20">
      <div className="text-center mb-8">
       
        <h2 className="text-2xl font-bold bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent">Selamat Datang</h2>
        <p className="text-slate-500 text-sm mt-1">Silakan lengkapi data diri untuk mulai mengobrol dengan AI</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div>
          ...
          <input
            {...register('name')}
            type="text"
            className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder:text-slate-400"
        ...
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-2 ml-1">Nomor WhatsApp</label>
          <input
            {...register('phone')}
            type="tel"
            className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder:text-slate-400"
        ...
        <button
          ...
          className="w-full bg-[#082a3d] text-white font-bold py-3.5 px-6 rounded-xl hover:shadow-[0_10px_20px_rgba(37,99,235,0.3)] hover:scale-[0.99] active:scale-[0.97] transition-all disabled:opacity-50 disabled:pointer-events-none mt-4 shadow-lg shadow-[#082a3d]/20"
        >
          {isSubmitting ? (
